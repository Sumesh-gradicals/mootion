...
const FloatingButton = () => {
  ...
  return (
    <div className="fixed bottom-24 left-1/2  -translate-x-1/2 transition-all rounded-full md:rounded-2xl text-white bg-[#383338] shadow-xl">
      {/* Slide-Up Transition */}
      <div
        className={` text-white  rounded-xl shadow-lg  transition-all    overflow-hidden${
          state.isExpanded ? " opacity-100 w-[425px] h-[380px] bg-[#262526] p-4" : " h-[0px] opacity-0"
        }`}
      >
        {state.isExpanded && (
            ...
            <p
              ...
              className={`cursor-pointer transition-all   ${
                state.selectedCategory === category
                  ? "text-white text-2xl font-bold italic scale-105"
